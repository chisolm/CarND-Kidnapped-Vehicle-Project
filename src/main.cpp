  ...
  h.onMessage([&pf,&map,&delta_t,&sensor_range,&sigma_pos,&sigma_landmark](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      // Predict the vehicle's next state from previous (noiseless control) data.
          ...
          // Add Ground truth output to plot performance 
          double sense_x = std::stod(j[1]["sense_x"].get<std::string>());
          double sense_y = std::stod(j[1]["sense_y"].get<std::string>());
          double sense_theta = std::stod(j[1]["sense_theta"].get<std::string>());
          cout << "Ground Truth\t" << sense_x << "\t" << sense_y << "\t" << sense_theta << endl;
      ...
      cout << "average w: " << weight_sum/num_particles << endl;
          ...
          cout << "best_particle\t" << msgJson["best_particle_x"] << "\t" << msgJson["best_particle_y"] << "\t" << msgJson["best_particle_theta"] << endl;
          cout << "best_particle_associations " << msgJson["best_particle_associations"] << endl;
